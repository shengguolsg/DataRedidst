...
!      
      CALL DESCINIT( DESCC, N, nev, MB, NB, 0, 0, CONTEXT, nb_rows, INFO )

!      write(*,*) "rank,nb_rows,nb_cols=", myid, nb_rows, nb_cols, nprocs
!      
      Msi = Ms(myid+1) 
      allocate( A(N,Msi),B(n,nb_cols),C(nb_rows,nb_cols)  )
      allocate( Xr(N,Msi) )
...
!     Call the routine 1DTo2D_DataRedist to obtain the BCDD form
      ...
      Call OneTo1D_ComplexDataRedist( N, A, N, msi,B, N, nb_cols, NB, Ms, nprocs, &
                                      mpi_comm_world )
